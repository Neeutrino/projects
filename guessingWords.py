...
# функция логики
def play(word_random):
    word_completion = "_" * len(word_random)
    guessed = False
    guessed_letters = []
    guessed_words = []
    tries = 6
    ...
    while not guessed and tries > 0:
        ...
        if len(guess) == 1 and guess.isalpha():
            if guess in guessed_letters:
                ...
                indices = [i for i in range(len(word_random)) if word_random[i] == guess]
                for index in indices:
                    word_as_list[index] = guess
                word_completion = ''.join(word_as_list)
                if '_' not in word_completion:
                    guessed = True
        elif len(guess) == len(word_random) and guess.isalpha():
            if guess in guessed_words:
                print('Вы уже называли слово', guess)
            elif guess != word_random:
                print('Слово', guess, 'не является верным.')
                tries -= 1
                guessed_words.append(guess)
            else:
                guessed = True
                word_completion = word_random
        else:
    ...
    if guessed:
        ...
        print('Вы не угадали слово. Загаданным словом было ' + word_random + '. Может быть в следующий раз!')
